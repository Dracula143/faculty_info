		...
		if(isset($_POST['upd']))
		{
			...
			{ 
				...
				if(mysqli_num_rows(mysqli_query($con, $sqlz)) != 1)
				{
    					...
    					if ($ufE == $uafE)
    					{
    						...
    						$unFN = "$ciu-$ucf";
      						...
      						if (!file_exists($udp))
      						{
      							...
      							{
									$wu12 = "UPDATE certifications SET cfile='$unFN', cname='$ucn', cindex='$uct', cjournal='$ucs', cvol='$uce', cyear='$ucy' where cuid='$uci' and cname='$uci1' and ctype='Conference' and cyear='$uci2' ";
					...
					if($_FILES['ucf']['error'] == UPLOAD_ERR_NO_FILE)
					{
						$wu21 = "UPDATE certifications SET cname='$ucn', cindex='$uct', cjournal='$ucs', cvol='$uce', cyear='$ucy' where cuid='$uci' and cname='$uci1' and ctype='Conference' and cyear='$uci2' ";
			...
			if($ciu == $uci && $ucn == $uci1 && $ucy == $uci2)
			{
    				...
    				if ($ufE == $uafE)
    				{
						...
						if (file_exists($udp))
      					...
      					if (!file_exists($udp))
      					{
       						if(move_uploaded_file($ufT, $udp)) 
      						{
								$wu1 = "UPDATE certifications SET cfile='$unFN', cname='$ucn', cindex='$uct', cjournal='$ucs', cvol='$uce', cyear='$ucy' where cuid='$uci' and cname='$uci1' and ctype='Conference' and cyear='$uci2' ";
				...
				if($_FILES['ucf']['error'] == UPLOAD_ERR_NO_FILE)
				{
					$wu2 = "UPDATE certifications SET cname='$ucn', cindex='$uct', cjournal='$ucs', cvol='$uce', cyear='$ucy' where cuid='$ciu' and cname='$uci1' and ctype='Conference' and cyear='$ucy' ";

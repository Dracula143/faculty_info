<!DOCTYPE html>
<html style=" background: url(Images/seminar.png) no-repeat center center fixed; -webkit-background-size: cover; -moz-background-size: cover; -o-background-size: cover; background-size: cover; margin: 0;">
<head>
	<meta charset="utf-8">
...
<body>
	...
	while($fd = mysqli_fetch_array($fr))
		{
			...
			$uidi = $fd["fuid"];
			$w = "SELECT * FROM certifications where ctype = 'Seminar' and cuid='$uidi' ";
			...
			<tr>
				...
				<th><a style="text-decoration: none;" href="adminseminar.php?upd=<?php echo $nam; ?>&upd1=<?php echo $nam1; ?>&upd2=<?php echo $nam2; ?>&upd3=<?php echo $nam3; ?>" id="upd" title="Edit Seminar"> Y </a></th>
                <th><a style="text-decoration: none;" onclick="javascript: return confirm('Do you wnat to delete Seminar <?php echo $wv["cname"]; ?>');" href="adminseminar.php?del=<?php echo $nam; ?>&del1=<?php echo $nam1; ?>&del2=<?php echo $nam2; ?>&del3=<?php echo $nam3; ?>" > X </a></th>
            </tr>
        <?php 
    	}
    }
	...
	<form class="f1" id="f1" action="" method="POST" style="display: none" enctype="multipart/form-data">
       <h3 class="h3">Insert Seminar</h3>
        ...
        <input class="i1" type="text" placeholder="Enter Seminar Title" name="cn" required><br>
    ...
    <?php
    	if(isset($_POST['add']))
		{
			...
			$cf = $_FILES['cf']['name'];
			$qsa = "SELECT * FROM certifications where cuid = '$ci' and ctype = 'Seminar' and cname='$cn' and cyear='$cy' and cstart='$cs' ";
			...
			if(mysqli_num_rows($qsar) != 1)
			{
    			...
    			if ($fE == $afE)
    			{
      				...
      				if (!file_exists($dp))
      				{
      					...
      					{
      						$sql2 = "INSERT into certifications SET cfile='$nFN', cname='$cn', cdur='$t', cstart='$cs', cend='$ce', cyear='$cy', ctype='Seminar', cuid='$ci'";
      						if (mysqli_query($con, $sql2))
      						{
      							echo '<script>alert("Seminar Added Successfully"); location.href="adminseminar.php"</script>';
      						}
      						else
      						{
      							echo '<script>alert("Error while Adding"); location.href="adminseminar.php"</script>';
      						}
      					}
      				}
      				if (file_exists($dp)) 
      				{
      					echo '<script>alert("File name Already exist. Please rename file and Try Again"); location.href="adminseminar.php";</script>';
      				}
      			}
      			if($fE != $afE)
      			{
      				echo '<script>alert("File extension must be pdf"); location.href="adminseminar.php";</script>';
      			}
      		}
      		if(mysqli_num_rows($qsar) == 1)
      		{
      			echo '<script>alert("Seminar Already exist."); location.href="adminseminar.php";</script>';
      		}
      	}
		if(!empty($_GET['upd']) && !empty($_GET['upd1']) && !empty($_GET['upd2']) && !empty($_GET['upd3']))
		{
			...
			$uci2 = $_GET['upd2'];
			$uci3 = $_GET['upd3'];
			$sql1 = "SELECT * FROM certifications where cuid='$uci' and cname = '$uci1' and ctype='Seminar' and cyear='$uci2' and cstart='$uci3' ";
			...
			<h3 class="h3">Update Seminar</h3>

			<label class="l1" > Unique ID </label>
        	<input class="i1" type="text" placeholder="Enter Unique ID of Faculty" name="ciu" value="'.$wud["cuid"].'" required> <br>

        	<label class="l1"> Title </label>
        	<input class="i1" type="text" placeholder="Enter Seminar Name" value="'.$wud["cname"].'" name="ucn" required> <br>
		...
		if(isset($_POST['upd']))
		{
			...
			{ 
				$sqlz = "SELECT * FROM certifications where cuid='$ciu' and cname = '$ucn' and ctype='Seminar' and cyear='$ucy' and cstart='$ucs' ";
				if(mysqli_num_rows(mysqli_query($con, $sqlz)) != 1)
				{
    					...
    					if ($ufE == $uafE)
    					{
    						...
    						$unFN = "$uid-$ucf";
      						...
      						if (!file_exists($udp))
      						{
      							...
      							{
									$wu12 = "UPDATE certifications SET cfile='$unFN', cname='$ucn', cdur='$ct', cstart='$ucs', cend='$uce', cyear='$ucy' where cuid='$uci' and cname='$uci1' and ctype='Seminar' and cyear='$uci2' and cstart='$uci3' ";
									if (mysqli_query($con, $wu12))
									{
										echo '<script>alert("Seminar Details Updated Successfully"); location.href="adminseminar.php"</script>';
									}
									else
									{
										echo '<script>alert("Error while updating"); location.href="adminseminar.php"</script>';
									}
								}
							}
							if (file_exists($udp))
					...
					if($_FILES['ucf']['error'] == UPLOAD_ERR_NO_FILE)
					{
						$wu21 = "UPDATE certifications SET cname='$ucn', cdur='$ct', cstart='$ucs', cend='$uce', cyear='$ucy' where cuid='$uci' and cname='$uci1' and ctype='Seminar' and cyear='$uci2' and cstart='$uci3'  ";
			...
			if($ciu === $uci && $ucn === $uci1 && $ucs === $uci3 && $ucy === $uci2)
			{
    				...
    				if ($ufE == $uafE) 
    				{
						...
						if (file_exists($udp))
      					...
      					if (!file_exists($udp))
      					{
       						if(move_uploaded_file($ufT, $udp)) 
      						{
								$wu1 = "UPDATE certifications SET cfile='$unFN', cname='$ucn', cdur='$ct', cstart='$ucs', cend='$uce', cyear='$ucy' where cuid='$ciu' and cname='$uci1' and ctype='Seminar' and cyear='$uci2' and cstart='$uci3' ";
				...
				if($_FILES['ucf']['error'] == UPLOAD_ERR_NO_FILE)
				{
					$wu2 = "UPDATE certifications SET cname='$ucn', cdur='$ct', cstart='$ucs', cend='$uce', cyear='$ucy' where cuid='$ciu' and cname='$uci1' and ctype='Seminar' and cyear='$ucy' and cstart='$ucs' ";
		...
		if(!empty($_GET['del']) && !empty($_GET['del1']) && !empty($_GET['del2']) && !empty($_GET['del3'])) //code to delete the selected Seminar
		{
			$m = $_GET['del'];
			$m1 = $_GET['del1'];
			$m2 = $_GET['del2'];
			$m3 = $_GET['del3'];
			$selectSql = "SELECT * from certifications where cuid='$m' and cname = '$m1' and ctype='Seminar' and cyear='$m2' and cstart='$m3' ";
			...
			{
				$sqle = "DELETE FROM certifications where cuid='$m' and cname = '$m1' and ctype='Seminar' and cyear='$m2' and cstart='$m3' ";
				if(mysqli_query($con, $sqle))
				{
					echo '<script>alert("Deleted Successfully"); location.href="adminseminar.php"</script>';
				}
				else
				{
					echo '<script>alert("Error while Deleting"); location.href="adminseminar.php"</script>';
				}
			}
		}
		?>
